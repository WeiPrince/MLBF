...
Model_Path_imdb = '../models_saved/lstm_kaggle.pt'
...
class LSTM(nn.Module):
    def __init__(self,  feature_sizes, feature_sections_dict = None, feature_sizes_dict = None, dim_hidden = 32,
        ...
        self.LSTM = nn.LSTM(input_size=dim_embedding,hidden_size=self.dim_hidden,num_layers = 1,
    ...
    def forward(self, src):
        ...
        h0 = torch.randn(1, src.size(0), 32).cuda()
        c0 = torch.randn(1, src.size(0), 32).cuda()
        result_useless,(hn,cn) = self.LSTM(src_embedded,(h0,c0))
        hn = hn.squeeze(0)
        result = self.linear(hn)
        return result
...
if __name__ == '__main__':
    ...
    lstm_model = LSTM(feature_sizes, dim_in=39)
    train(lstm_model, Model_Path_imdb, loader_train, loader_val, iter_time=100)
    check_accuracy(loader_test, lstm_model, False)
    '''
    ...
    lstm_model = LSTM(feature_sizes, feature_sections_dict, feature_sizes_dict, dim_embedding = 256, dim_in = 45, dim_out = 1)
    train(lstm_model, Model_Path_imdb, loader_train, loader_val, iter_time = 100)
    check_accuracy(loader_test, lstm_model, False)
    result = check_accuracy(loader_all, lstm_model, False)
    df_result = pd.DataFrame(np.array(result).reshape(-1, 1))
    df_result.to_csv('lstm_imdb.csv')
    '''
